...
st.markdown("""
    ...
    /* 縮小下方三個 st.metric (市值/成本/總領息) 的字體與邊距 */
    .stMetric { background-color: #f8f9fa; padding: 8px 12px; border-radius: 8px; margin-bottom: -10px; }
    ...
    /* 大幅縮小今日損益、預估領息這三個框框的內邊距與字體 */
    .triple-box { background-color: #ffffff; border-radius: 10px; border: 1px solid #e0e0e0; padding: 10px 12px; display: flex; flex-wrap: wrap; justify-content: space-around; align-items: center; margin-bottom: 12px; box-shadow: 2px 2px 6px rgba(0,0,0,0.03); gap: 8px; }
    .triple-col { flex: 1 1 30%; min-width: 140px; text-align: center; padding: 5px 0; }
    .triple-title { font-size: 12px; color: #757575; font-weight: bold; margin-bottom: 2px; }
    .triple-val-r { font-size: 22px; font-weight: 900; color: #d32f2f; font-family: Arial, sans-serif; line-height: 1.1; }
    .triple-val-g { font-size: 22px; font-weight: 900; color: #388e3c; font-family: Arial, sans-serif; line-height: 1.1; }
    .triple-val-gold { font-size: 22px; font-weight: 900; color: #f39c12; font-family: Arial, sans-serif; line-height: 1.1; text-shadow: 1px 1px 2px rgba(243, 156, 18, 0.3); }
    .triple-pct-r { font-size: 12px; font-weight: bold; color: #d32f2f; margin-top: 3px; }
    .triple-pct-g { font-size: 12px; font-weight: bold; color: #388e3c; margin-top: 3px; }
    .triple-sub-gold { font-size: 11px; font-weight: bold; color: #7f8c8d; margin-top: 3px; }

    @keyframes lightning-strike {
        0% { box-shadow: 0 0 8px rgba(241, 196, 15, 0.4); background-color: #fffdf5; border-color: #f1c40f; transform: scale(1); }
        50% { box-shadow: 0 0 25px rgba(255, 235, 59, 0.8), inset 0 0 15px rgba(255, 235, 59, 0.7); background-color: #ffffe0; border-color: #ffeb3b; transform: scale(1.02); }
        100% { box-shadow: 0 0 8px rgba(241, 196, 15, 0.4); background-color: #fffdf5; border-color: #f1c40f; transform: scale(1); }
    }
    .flash-gold-box { background-color: #fffdf5; border-radius: 10px; padding: 10px; border: 2px solid #f1c40f; animation: lightning-strike 0.1s infinite; }
...
# --- 5. 介面呈現 ---
# ================================
# 🔥 修改：精簡版標題與最後更新時間 (併排顯示)
# ================================
st.markdown(f"""
    <div style="display: flex; align-items: baseline; gap: 15px; margin-bottom: 10px; margin-top: -20px;">
        <span style="font-size: 22px; font-weight: 900; color: #2c3e50;">📈 實戰資產戰情室</span>
        <span style="font-size: 13px; color: #7f8c8d;">最後更新：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</span>
    </div>
""", unsafe_allow_html=True)

# ================================
# 🔥 修改：將美股與台股大盤全部整合成上方橫向捲動的迷你小卡
# ================================
mini_html = '<div class="mini-card-container">'

# 1. 放入美股關鍵指標
if "us" in macro_data and macro_data["us"]:
    for name, d in macro_data["us"].items():
        is_up = d['diff'] >= 0
        color_hex = "#d32f2f" if is_up else "#388e3c" # 紅漲綠跌
        sign = "▲" if is_up else "▼"
        mini_html += f"<div class='mini-card'><div class='mini-title'>🇺🇸 {name}</div><div class='mini-price' style='color: {color_hex};'>{d['price']:,.2f}</div><div class='mini-pct' style='color: {color_hex};'>{sign} {abs(d['diff']):.2f} ({abs(d['pct']):.2f}%)</div><div class='mini-time'>🕒 {d['date']}</div></div>"

# 2. 放入台股關鍵指標
if "tw" in macro_data and macro_data["tw"]:
    for name, d in macro_data["tw"].items():
        is_up = d['diff'] >= 0
        color_hex = "#d32f2f" if is_up else "#388e3c"
        sign = "▲" if is_up else "▼"
        mini_html += f"<div class='mini-card'><div class='mini-title'>🇹🇼 {name}</div><div class='mini-price' style='color: {color_hex};'>{d['price']:,.2f}</div><div class='mini-pct' style='color: {color_hex};'>{sign} {abs(d['diff']):.2f} ({abs(d['pct']):.2f}%)</div><div class='mini-time'>🕒 {d['date']}</div></div>"
...
st.markdown(f"""
    <div style="text-align: center; margin-bottom: 6px; margin-top: -10px;">
        <span style="color: #666; font-size: 15px; font-weight: bold; margin-right: 8px;">🏆 總累計領取配息</span>
        <span style="color: #ff4b4b; font-size: 24px; font-weight: 900;">${total_collected:,.0f}</span>
    </div>
""", unsafe_allow_html=True)

# 切割成 4 個區塊，讓兩個按鈕緊湊地置中並排
col_space1, col_add, col_sub, col_space2 = st.columns([1, 1.5, 1.5, 1])

with col_add:
    if st.button(f"➕ 加入本月預估 (${current_month_div_amount:,.0f})", use_container_width=True, type="primary"):
        st.session_state.my_data['total_collected_dividends'] = total_collected + current_month_div_amount
        save_to_json(st.session_state.my_data)
        st.rerun()

with col_sub:
    if st.button(f"↩️ 誤觸復原 (扣除)", use_container_width=True):
        # 防呆機制：確保扣除後不會變成負數
        new_total = total_collected - current_month_div_amount
        st.session_state.my_data['total_collected_dividends'] = max(0, new_total) 
        save_to_json(st.session_state.my_data)
        st.rerun()

# ⚠️ 注意：我把原本在這裡的 st.write("") 刪掉了，進一步節省下方多餘的空白
...
# ================================
# 🔥 修改：移除美股/台股按鈕，剩下的6個功能按鈕排成 2x3 網格
# ================================
...
with cols_btn_r2[2]: st.button(b9_lbl, on_click=toggle_history, type=b9_typ, use_container_width=True) 
...
if st.session_state.show_tech:
    if not df.empty:
        ...
        st.info("💡 如果 yfinance 昨收價不準，請直接雙擊表格中的 **「昨收價(雙擊修改)」** 數字來修正！(改為 0 即恢復自動抓取)")
        ...
        edited_tech = st.data_editor(
            styled_df_tech,
            column_config={
                # 🔥 這裡把標題改短，並加上 width="small" 來強制縮小欄寬
                "昨日收盤價": st.column_config.NumberColumn("昨收價(雙擊修改)", min_value=0.0, format="%.2f", width="small"),
                "現價": st.column_config.NumberColumn("現價", format="%.2f", width="small"),
                "均價": st.column_config.NumberColumn("持有均價", format="%.2f", width="small"), # 🔥 把均價排在現價旁邊
                "股票張數": st.column_config.NumberColumn("庫存(張)", format="%.3f", width="small") 
            },
            disabled=["ETF 名稱", "股票張數", "現價", "均價", "今日損益", "今日漲跌", "今日漲跌幅", "今日交易量", "今日最高/最低", "52週最高/最低"],
            use_container_width=True, hide_index=True
        )

        has_changes = False
        for i, row in edited_tech.iterrows():
            display_name = row['ETF 名稱']
            new_prev = row['昨日收盤價']  # 🔥 這裡修復了 KeyError，對應回正確的內部欄位名稱
            orig_prev = df_tech.loc[i, '昨日收盤價'] 

            if new_prev != orig_prev:
                for etf in st.session_state.my_data['etfs']:
                    if etf['name'] in display_name:
                        etf['manual_prev_price'] = float(new_prev)
                        has_changes = True
                        break
